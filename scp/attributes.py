...
def lizard_analysis(dir_path: str) -> Tuple[pd.DataFrame, List[str]]:
    # print(1)
    analysis_results = lizard.analyze([dir_path])
    print("lizard path at: ", dir_path)
    # Prepare data for each file analyzed
    results = []
    for file_info in analysis_results:
        # print(file_info.__dict__)
        total_nloc = file_info.nloc
        avg_nloc = file_info.average_nloc
        avg_ccn = file_info.average_cyclomatic_complexity
        avg_token = file_info.token_count
        function_cnt = len(file_info.function_list)

        # Append data for each file to the results list
        results.append({
