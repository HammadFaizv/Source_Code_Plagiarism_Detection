...
def greedyTiling(treeA: javalang.tree.CompilationUnit, treeB: javalang.tree.CompilationUnit) -> float:
    """
    ...
    """
    treelistA = nodeutil.convertToList(treeA)  # check type of tree please
    treelistA = nodeutil.convertToList(treeA) # check type of tree please
    ...
    return calculateScore(tiles, treelistA, treelistB) # why breaks?

def calculateScore(matchlist: list, treelistA: list, treelistB: list) -> float:
    """
        Calculates similarity score for 2 ast's based on the matches
        
        Returns:
            float: a score indicating the similarity between the two ASTs
    """
