...
def quick_gst(filename_1:str, filename_2:str, hash_map:dict, tree_map:dict) -> int:
    """
    Computes the similarity score between two files using a quick version of the Greedy String Tiling algorithm.

    Args:
        filename_1 (str): The name of the first file.
        filename_2 (str): The name of the second file.
        hash_map (dict): A dictionary mapping filenames to their corresponding hash tables.
        tree_map (dict): A dictionary mapping filenames to their corresponding tokenized parse trees.

    Returns:
        int: The similarity score between the two files, ranging from 0 to 1, where 1 indicates identical files.

    The function ensures that the smaller of the two token lists is used for traversal to optimize performance.
    It identifies and marks maximal matches between the two token lists, and calculates the similarity score
    based on the length of these matches.
    """
...
def main(source_dir, output_dir, outbox, limit = None):
    ...
    for filename_1, filename_2 in tqdm(itertools.combinations(tree_map, 2)):
