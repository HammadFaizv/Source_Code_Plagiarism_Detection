...
SPLIT_DF = 50000
curr_directory = constants.SOCO_TRAIN
df_name = "soco_all"
limit = None # just 600 files from original
...
for filename in tqdm(javadir):
    ...
    try:
        ...
        # file_data_file.write(str(temp) + "\n")
        files.append(str(filename))
        # files_file.write(str(filename) + "\n")
...
print(f"[{len(files)}] have been scanned.")
...
print('attribute_df shape', attribute_df.shape)

# Drops Nan values
# Uncomment # @then gets rid of anomalies by classifying everything greater than 3 standard deviations away from the
# mean as an anomaly
# print("attribute shape before",attribute_df.shape)
# attribute_df.dropna(inplace=True) # this is the problem; everything has something null in them
...
# print("attribute shape after",attribute_df.shape)
# attribute_df.to_csv("dataframes/attribute_df_soco_a1.csv", index=False)
...
# print("full_df_rows", full_df_rows)

# Calculating scores using improved GST for every pair of files
score_map = quick_gst(curr_directory[:-1], None, None, limit) # takes the most amount of time
# print(score_map)
...
for file1_att, file2_att in itertools.combinations(full_df_rows, 2):
    ...
    if size_count >= SPLIT_DF:
        filename = "./scp/dataframes/" + df_name + "_final_train_part_" + str(iteration_no) + ".csv"
        iteration_no += 1
        size_count = 0
